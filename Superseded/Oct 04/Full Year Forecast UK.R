...
Reprint <- Reprint[,c(grep("ASIN", colnames(Reprint)), grep("Available.Inventory", colnames(Reprint)), 
                      grep("Stock.at.TBS", colnames(Reprint)), grep("Reprint.Dates", colnames(Reprint)))]
...
prev_year <- prev_year[,c(1,3,grep("2020-01-04", colnames(Q1)): grep("2021-01-02", colnames(Q1))) ]
...
prev_year <- prev_year[,c(1,3,grep("2020-01-04", colnames(Q1)): grep("2021-01-30", colnames(Q1))) ]
prev_year[prev_year <= 0] <- 1


#Create empty data frame to store seasonal percentage changes
Seas_adjQ <- data.frame(matrix(ncol = 59 , nrow = nrow(prev_year)))
colnames(Seas_adjQ) <- colnames(prev_year)
...
DF$inventory <- DF$`Amz inv` + DF$`TBS inv`
DF$Inv_issue <- 0
DF$WOH <- "12+"
...
forecasting_statistics_temp <- pred_df_holt_damp_beta[,c(1:22,32,31,28,30,23,24,26,27,25)]
pred_df_holt_damp_beta <- pred_df_holt_damp_beta[,c(1:22,32,31,28,30,23,24,26,27)]
...
write.csv(pred_df_holt_damp_beta, paste0("Forecast uk - ",pred_date,".csv"), row.names = F)
